...
const AddSubject = () => {
    ...
    try {
      ...
      if (isEdit) {
        response = await axios.put(
          `https://pragati-2-0.onrender.com/api/materials/${editId}`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://pragati-2-0.onrender.com/api/materials/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
                ...
                filteredSubjects.map((sub, index) => (
                  <tr key={sub._id || index}>
                    ...
                    <td>
                      {sub.fileUrl ? (
                        <a
                          href={sub.fileUrl.startsWith("http") ? sub.fileUrl : `https://pragati-2-0.onrender.com${sub.fileUrl}`}
                          target="_blank"
